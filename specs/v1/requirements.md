# Cup of IQ — Requirements (v2, 2026-07-08 · GRN-1 and LCK amended 2026-08-03)
...
14. **Number-word audio ships in MVP.** On each correct tap a recorded "hatchling voice" speaks the number word. Parent-produced recording; **no text-to-speech in the product**. The wrong-tap voice line remains deferred to Phase 2.
...
17. **Grown-ups entry is a plain tap (supersedes the long-press in GRN-1).** The 2 s continuous press proved unusable one-handed on a phone: roughly four times the platform norm, no progress feedback, a small target, cancelled by any finger drift or by the scroller claiming the gesture. The panel was never a secret, and its one destructive action keeps its own confirm (GRN-3). **Accepted risk:** a child can tap in and change the level; the change applies from the next round and is reversible in seconds.
18. **Replays return, first attempt only (reverses the original once-a-day resolution).** Observed: he asks to see the dino and play again; he lights up for tapping the patches, the egg cracking, and the dance; he gets 1→2→3 with a moment's thought — so the difficulty is right and the *dose* is short. A "Play again" control on both the results screen and the come-back card runs the round again with a fresh scatter and the same dinosaur, recording nothing. **What is preserved:** one species per day, one recorded result, one shared result, level-up driven only by first attempts, and the day still *opening* on the come-back card. **What is given up:** the come-back card no longer ends the "AGAIN!" negotiation by itself, and screen time is no longer bounded by the app. That bound moves to the parent, deliberately.
...
- **DPS-3** WHEN the board is built for the day's first round at level *L* on day *N*, THE SYSTEM SHALL derive the assignment of values to layout slots from a seeded PRNG with seed `N * 100 + L`, so every device at the same level sees the identical arrangement that day. Replays (LCK-5) SHALL use a different seed per attempt and are exempt: they are neither recorded nor shared, so they need no cross-device agreement.
...
- **AUD-4** THE SYSTEM SHALL initialize/unlock the audio context on the first tap gesture, and SHALL lazy-load audio after first paint so NFR-2's first-render budget is unaffected.
- **AUD-5** WHEN the hatch occurs, THE SYSTEM SHALL play the celebratory hatchling "rawr" — identical every day, at every accuracy, and on every replay (CEL-2).
...
- **CEL-2** THE SYSTEM SHALL play the identical child-facing celebration regardless of `wrongTaps` or attempt number; accuracy SHALL alter only grown-up-facing surfaces.
...
- **SHR-1** WHEN the results screen renders, THE SYSTEM SHALL show the day's **recorded first attempt**: today's dinosaur, `wrongTaps`, the dino title per the ladder (0 → T-Rex; 1 → Triceratops; 2–3 → Stegosaurus; 4+ → Brontosaurus), and the level played. A replay SHALL NOT change any of these.
- **SHR-2** WHEN the recorded `wrongTaps` is 0, THE SYSTEM SHALL show distinct perfect-round treatment on grown-up surfaces only: confetti burst, ⭐ badge, and progress toward level-up (e.g., "2 of 3 perfect days").
...
- **LCK-1** WHEN today's round has already been completed on this device, THE SYSTEM SHALL open on the come-back card rather than the board: today's dino pictured statically, a warm "New egg tomorrow" message, and the share/copy controls for today's result. The card remains the day's default landing state — replaying is an explicit choice, never the automatic one.
- **LCK-2** THE come-back card SHALL contain no interactive elements other than the share/copy controls, the grown-ups control, and the Play again control (LCK-5). No collection shelf, no toys, no free-play surface.
...
- **LCK-4** WHEN a round is in progress and the page is reloaded the same day, THE SYSTEM SHALL restart that round from the beginning (in-progress state is not persisted), and this SHALL not count as a completed day. (Accepted per sign-off #9.)
- **LCK-5** *(sign-off #18)* WHEN the Play again control is pressed on the results screen or the come-back card, THE SYSTEM SHALL run a further round at the current level with a freshly shuffled arrangement and the same dinosaur, and SHALL record nothing: `wrongTaps` is discarded, `perfectsAtLevel` is unchanged, no level-up is evaluated, `lastPlayed` is not overwritten, and the shared text remains the first attempt's. Replays SHALL be unlimited in number and SHALL be indistinguishable from the first round to the child (CEL-2).
- **MSN-1** *(Phase 2)* WHEN the come-back card renders, THE SYSTEM SHALL show the day's real-world counting mission selected as `missions[(dayNumber - 1) % missions.length]` from `content/missions.json`, matched to the child's current level range where practical.
...
- **NFR-2 (performance)** First playable render SHALL occur within 2 s on a mid-range phone over fast 3G; total initial transfer including today's dino image SHALL be ≤ 300 KB gzipped. Each dino image SHALL be WebP and ≤ 60 KB. Audio files SHALL be lazy-loaded after first paint; each ≤ 25 KB. Replays SHALL fetch nothing further (same day, same dino, same audio).
